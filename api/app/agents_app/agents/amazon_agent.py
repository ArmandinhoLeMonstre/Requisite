...
async def call_amazon_agent(object_type: str, object_specs: str):
            ...
            if item.name == "get_amz_product_list":
                ...
                if func is None:
                    raise SubAgentError(
                        message="Amazon agent did not call any tool",
                        agent= "amazon_agent",
                        action= "Inform the user that there is a problem with the notifier agent, he can to retry later",
                        step="Tool call"
                    )
    ...
    try:
        ...
        raise SubAgentError(
            message=str(e),
            agent= "amazon_agent",
            action= "Inform the user that there is a problem with the notifier agent, he has to retry later, any amazon search won't work",
